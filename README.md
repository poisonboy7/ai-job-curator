...
이 프로젝트는 사용자의 이력서를 AI가 분석하여, 선택한 지역과 조건에 맞는 채용 공고를 매일 아침 메일로 큐레이션해 주는 자동화 시스템입니다. 누구나 순서대로 따라 하면 내 컴퓨터에 같은 시스템을 구축할 수 있습니다.

### 현재 지역 매칭 방식

- 주소 검색 결과에서 주요 시·구·군 지역명을 추출합니다.
- 채용공고의 주소에 해당 지역명이 포함되는지를 기준으로 필터링합니다.
